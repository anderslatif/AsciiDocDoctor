...
import { validateAsciiDocTable } from './optionHandlers/validateTables.js';
...
.option('--validate-tables', 'Validates that tables are defined with valid syntax: The right amount of columns according to the header.')
...
directoriesPaths.map(async (directoryPath) => {
  ...
  filePathsInDirectory.map(async (filePath) => {
    ...
    if (options.validateTables || options.all) {
      const isValidTable = validateAsciiDocTable(fileContent);
      if (!isValidTable) {
        console.log(`Invalid table structure in file: ${filePath}`);
      }
    }
  });
